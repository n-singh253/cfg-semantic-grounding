...
SPLIT="${SPLIT:-test}"
ATTACKS="${ATTACKS:-none fcv_cwe78_base64_obfuscated swexploit_base64_obfuscated}"
SHARDS="${SHARDS:-8}"
PARALLEL="${PARALLEL:-4}"
DRY_RUN="${DRY_RUN:-0}"
...
run_cmd() {
  echo
  printf '[featurebench-base64-obfuscated-datasets] run:'
  printf ' %q' "$@"
  echo
  if [[ "$DRY_RUN" == "1" ]]; then
    return 0
...
require_file() {
  local path="$1"
  if [[ ! -f "$path" ]]; then
    echo "[featurebench-base64-obfuscated-datasets] missing required file: $path" >&2
    exit 1
  fi
}
...
set_claude_env() {
  if [[ -z "${ANTHROPIC_API_KEY:-}" ]]; then
    echo "[featurebench-base64-obfuscated-datasets] ANTHROPIC_API_KEY is required for CLAUDE_AGENT=$CLAUDE_AGENT" >&2
    exit 1
  fi
  # Obfuscated SWExploit uses Gemini Vertex for attack prompt generation; keep
  # Vertex env available while routing Claude SWE-agent calls through Anthropic.
  if [[ " $ATTACKS " == *" swexploit_base64_obfuscated "* ]]; then
    set_common_vertex_env
  fi
}
...
echo "[featurebench-base64-obfuscated-datasets] dataset=$DATASET split=$SPLIT attacks=$ATTACKS shards=$SHARDS parallel=$PARALLEL run_in_parallel=$RUN_IN_PARALLEL"
echo "[featurebench-base64-obfuscated-datasets] project=${GOOGLE_CLOUD_PROJECT:-$PROJECT} credentials=${GOOGLE_APPLICATION_CREDENTIALS:-$HOME/.config/cfg-semantic-grounding/gemini_adc.json}"
...
echo "[featurebench-base64-obfuscated-datasets] completed"
