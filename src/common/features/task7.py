"""Task7 feature extraction (structural + optional universal severity)."""
...
def extract_task7_features(record: Dict[str, Any], mode: str) -> Dict[str, float]:
...
def columns_for_task7_mode(mode: str) -> List[str]:
    if mode == "task7_severity_only_universal":
        return SEVERITY_ONLY_FEATURES_UNIVERSAL.copy()
    if mode == "task7_no_security":
        return [col for col in FEATURE_COLUMNS if not is_security_feature(col)]
    if mode == "task7_full_universal":
        return FEATURE_COLUMNS.copy()
    raise ValueError(f"Unsupported task7 mode: {mode}")
